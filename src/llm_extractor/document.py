...
def extract_document(image_bytes: bytes, provider_name: str | None = None
    ...
    messages = [
        ...
        LLMMessage(role="user", content="Extract the key fields from this document image."),
    ]
    # images=[...] is the multimodal part; the seam handles provider differences
    return get_provider(provider_name).parse(messages, DocumentExtraction, images=[image_bytes])